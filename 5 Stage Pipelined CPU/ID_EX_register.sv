module ID_EX_register (regWrite_in, regWrite_out, MemRead_in, MemRead_out, MemWrite_in, MemWrite_out, MemtoReg_in, MemtoReg_out,
								ALUsrc_in, ALUsrc_out, ALUOp_in, ALUOp_out, set_flags_in, set_flags_out, Rn_data1_in, 
								Rn_data1_out, Rm_data2_in, Rm_data2_out, immediate_in, immediate_out, Rd_in, Rd_out, Rm_in, Rm_out, Rn_in, Rn_out, reset, clk, 
								link_in, link_out, PC_4_in, PC_4_out);
	// **Pass in imm_or_Daddr as immeadiate_in
	
	//control signals
	input logic regWrite_in, MemRead_in, MemWrite_in, MemtoReg_in, ALUsrc_in, set_flags_in, link_in;
	output logic regWrite_out, MemRead_out, MemWrite_out, MemtoReg_out, ALUsrc_out, set_flags_out, link_out;
	input logic clk, reset;
	input logic [2:0] ALUOp_in;
	output logic [2:0] ALUOp_out;
	
	input logic [63:0]  Rn_data1_in, Rm_data2_in, immediate_in, PC_4_in; 
	output logic [63:0] Rn_data1_out, Rm_data2_out, immediate_out, PC_4_out;
...
//	D_FF Branch_DFF       (.q(Branch_out),       .d(Branch_in),       .reset(reset), .clk(clk));
	...
	register #(64) imm (.data_in(immediate_in), .wr_en(1), .data_out(immediate_out), .clk, .reset);
...
//	register #(64) BrReg (.data_in(BrReg_in), .wr_en(1), .data_out(BrReg_out), .clk, .reset);
	register #(64) Rn_data (.data_in(Rn_data1_in), .wr_en(1), .data_out(Rn_data1_out), .clk, .reset);
	register #(64) Rm_data (.data_in(Rm_data2_in), .wr_en(1), .data_out(Rm_data2_out), .clk, .reset);
	register #(5) Rd_reg (.data_in(Rd_in), .wr_en(1), .data_out(Rd_out), .clk, .reset);
	register #(5) Rn_reg (.data_in(Rn_in), .wr_en(1), .data_out(Rn_out), .clk, .reset);
	register #(5) Rm_reg (.data_in(Rm_in), .wr_en(1), .data_out(Rm_out), .clk, .reset);
